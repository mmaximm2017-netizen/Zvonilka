            ...
            Row(Modifier.fillMaxWidth().padding(horizontal=16.dp,vertical=2.dp),verticalAlignment=Alignment.CenterVertically) {
                Row(Modifier.widthIn(max=250.dp).clip(RoundedCornerShape(12.dp)).background(MaterialTheme.colorScheme.surfaceVariant).padding(3.dp),horizontalArrangement=Arrangement.spacedBy(2.dp)) {
                    listOf("Все","Пропущенные").forEachIndexed { index,label ->
                        ...
                        Surface(onClick={missedOnly=index==1},modifier=Modifier.width(if(index==0)92.dp else 130.dp),shape=RoundedCornerShape(9.dp),color=if(active) MaterialTheme.colorScheme.surface else Color.Transparent,shadowElevation=if(active) 1.dp else 0.dp) {
    ...
    @Composable private fun Keypad() {
        ...
        Column(Modifier.fillMaxSize().padding(horizontal=16.dp)) {
            ...
            Row(Modifier.fillMaxWidth(),verticalAlignment=Alignment.CenterVertically) {
                Text(NumberTools.display(number).ifBlank { "Введите номер" },Modifier.weight(1f).combinedClickable(onClick={},onLongClick={
                ...
                }),fontSize=if(number.isBlank())20.sp else 28.sp,fontWeight=if(number.isBlank())FontWeight.Normal else FontWeight.Medium,color=if(number.isBlank()) MaterialTheme.colorScheme.onSurfaceVariant else MaterialTheme.colorScheme.onSurface,maxLines=1)
                Box(Modifier.size(44.dp).combinedClickable(onClick={number=number.dropLast(1);picked=false},onLongClick={number="";picked=false}),contentAlignment=Alignment.Center) { Icon(Icons.Default.Backspace,"Удалить; удерживать для очистки",Modifier.size(22.dp)) }
            }
            if(matches.isNotEmpty()) {
                LazyColumn(Modifier.fillMaxWidth().heightIn(max=116.dp).clip(RoundedCornerShape(16.dp)).background(MaterialTheme.colorScheme.surface),contentPadding=PaddingValues(vertical=2.dp)) {
            ...
            if(p!=null) Row(Modifier.align(Alignment.CenterHorizontally).padding(top=4.dp,bottom=4.dp),verticalAlignment=Alignment.CenterVertically) { Photo(p,Modifier.size(28.dp).clip(CircleShape));Spacer(Modifier.width(7.dp));Text(p.name,style=MaterialTheme.typography.titleMedium) }
            else if(number.isNotBlank() && matches.isEmpty()) Row(Modifier.horizontalScroll(rememberScrollState())) {
                ...
                line.forEach { digit->
                    Surface(Modifier.padding(vertical=3.dp).size(68.dp).combinedClickable(onClick={
                        ...
                        Column(horizontalAlignment=Alignment.CenterHorizontally,verticalArrangement=Arrangement.Center) {
                            Text(digit.toString(),fontSize=29.sp,fontWeight=FontWeight.Normal,color=MaterialTheme.colorScheme.onSurface)
                            Text(when(digit){'2'->"ABC\nАБВГ";'3'->"DEF\nДЕЁЖЗ";'4'->"GHI\nИЙКЛ";'5'->"JKL\nМНО";'6'->"MNO\nПРС";'7'->"PQRS\nТУФХ";'8'->"TUV\nЦЧШЩ";'9'->"WXYZ\nЪЫЬЭЮЯ";'0'->"+";else->""},fontSize=9.sp,lineHeight=10.sp,textAlign=androidx.compose.ui.text.style.TextAlign.Center,color=MaterialTheme.colorScheme.onSurfaceVariant)
                        }
                    }
                }
            } }
            ...
            Column(Modifier.align(Alignment.CenterHorizontally).padding(top=8.dp,bottom=5.dp),horizontalAlignment=Alignment.CenterHorizontally) {
                Box(Modifier.size(70.dp).clip(CircleShape).background(Green).combinedClickable(onClick={dial(number)},onLongClick={Dialing.choose(this@MainActivity,number.ifBlank { null }){simRevision++}}),contentAlignment=Alignment.Center) {
